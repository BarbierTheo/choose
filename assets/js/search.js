...
if (movieName) {
  fetch(`https://api.themoviedb.org/3/search/movie?query=${movieName}&include_adult=false&language=fr-FR&page=1`, options)
    ...
    .then(movies => {
      const cardstack = document.getElementById('card-similar-section')
      cardstack.innerHTML = ""
      ...
      movies.results.forEach(film => {
        ...
        if (film.poster_path == null) {
          cardstack.innerHTML += `<a href="./movie.html?idMovie=${film.id}" class="flex flex-col gap-1 h-fit">
               <div class="w-[14rem] h-[18rem] rounded-lg skeleton"></div>
...
document.getElementById('searchtoggle').addEventListener('click', () => {
  ...
  // const select = document.getElementById('genrefilm')
  // const genreId = select.getAttribute('data-genre-ids')
  // console.log(genreId)
